...
def find_destory_order(location: (int, int), peers: [(int, int)]) -> list:
    peers_by_angle = bucket_peers_by_angle(location, peers)
    for peers_at_angle in peers_by_angle.values():
        peers_at_angle.sort(key=lambda peer: abs(peer[0] - location[0]) + abs(peer[1] - location[1]))

    max_rotations = max([len(i) for i in peers_by_angle.values()])
    destroyed = []
    for _ in range(max_rotations):
        for angle in sorted(peers_by_angle.keys(), key=angle_sort_key):
...
if __name__ == '__main__':
    ...
    part_2 = destroy_order[199] # 200th entry
    print(f"Solution to part 2 is {part_2}")
    assert part_2[0] * 100 + part_2[1] == 829
